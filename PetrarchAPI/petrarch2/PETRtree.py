...
class Phrase:
    ...
    def get_meaning(self):
        ...
        if self.label in "SBAR":
            ...
            filtered = filter(lambda a: a.label in "SBARVP",self.children)
            mapped = map(lambda a: a.get_meaning(), list(filtered))
...
#            actors = map( lambda a : mix( a[0], ag[1:]), actors)
            actors = list(map(lambda a: mix(a, ag[1:]), actors))

# --        print('mc-1',actors)
        return list(filter(lambda a: a not in ['', '~', '~~', None], actors))
...
# --        print('WTF-1')
    ...
    def get_head(self):
        ...
        try:
            if self.label == 'S':
                self.head, self.head_phrase = list(map(
                    lambda b: b.get_head(), list(filter(
...
class VerbPhrase(Phrase):
        ...
        try:
            if self.children[0].label == "VBN":
                ...
                if ((not (self.parent.get_head()[0] in helping or self.parent.children[0].text in helping)) and
                    len(list(filter(lambda a: isinstance(a, VerbPhrase), self.parent.children))) <= 1 and
    ...
    def get_meaning(self):
        ...
        s_options = list(filter(lambda a: a.label in "SBAR", self.children))
        ...
        lower = list(map(lambda a: a.get_meaning(), s_options))
        sents = []
...
#        self.get_lower = self.return_lower
        ...
        lower = list(map(lambda a: a.get_meaning(), v_options))
...
#        self.get_code = self.return_code
        ...
        if 'AND' in list(map(lambda a: a.text, self.children)):
            return 0, 0, ['and']
...
# --              print(match)
    ...
    def match_transform(self, e):
        ...
        def recurse(pdict, event, a2v={}, v2a={}):
            ...
            if isinstance(event, tuple):
                ...
                masks = list(filter(lambda a: a in pdict, [event[2], event[2] - event[2] % 0x10,
    ...
    def match_pattern(self):
        ...
        def match_phrase(path, phrase):
            ...
            if not phrase:
                return False
            for item in list(filter(lambda b: b.text in path, phrase.children)):
...
# --              print('mn-reroute2')
        ...
        def match_prep(path, phrase=self):
            # Matches preposition
            # --              print('mp-entry')
            for item in list(filter(lambda b: isinstance(
...
#--         print('si:==>',self.tree.get_parse_string())  # -- debug
    ...
    def get_metadata(self, entry):
        ...
        while id(next) in metadict:
            ...
            next = store[0]
        return list(map(lambda a: a[-2] if len(a) > 1 else a[0], meta_total[::-1]))
    ...
    def get_events(self, require_dyad=1):
        ...
        if PETRglobals.NullVerbs or PETRglobals.NullActors:
            utilities.nulllist = []
        filtered_list = list(filter(lambda b: b.label in "SVP",self.tree.children))

        events = map(lambda a: a.get_meaning(), filtered_list) # which is to say, label is (S or (VP
